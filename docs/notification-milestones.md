...
| **Függ** | N1.5 |
| **Állapot (2026-08-01)** | **KÉSZ.** 7 új végpont, a harang a Topbarban, a preferencia-UI a Settings oldalon. **387/387 teszt zöld** (26 fájl, 49,7 s) — 42 új: `notificationApi.test.ts` 34, `tenantIsolation.test.ts` +6, `notifications.test.ts` +2 regresszió. Mindkét build, mindkét lint tiszta. Séma-változás **nincs**, tehát nincs migráció és nincs mit drift-ellenőrizni. A mount-sorrendet a *lefordított* szerveren ellenőriztem (`dist`, adatbázis nélkül): a `/notifications/webhook/resend` továbbra is a nyers-body-s publikus webhookhoz jut el és nem a mögé mountolt auth-routerhez — ezt egyetlen teszt sem fedi, mert a suite az `app.ts`-t mountolja és nem nézi, *melyik* mount válaszolt. Az UI-t futó dev szerveren, stub API ellen ellenőriztem: harang + badge, olvasottra jelölés, „összes olvasott", lapozás, a 3 szintű feloldás mindkét scope-ban, mindkét nyelven. |
| **Eltérések a tervtől** | (1) **A router NEM `tenantWrite`-on megy** (terv §14), csak `authMiddleware`-en. A read-only mód számlázási állapot: a *cégadat szerkesztését* tiltja. Egy értesítés olvasottra jelölése nem az — az a felhasználó saját UI-állapota, és épp a read-only cég tulajdonosának van olvasatlan „lejárt a próbaidőszak" értesítése, amit el kell tudnia tüntetni. Ellenkező esetben a badge örökre ott maradna azon a képernyőn, amit a leginkább el akarunk olvastatni vele. Ugyanaz az indoklás, ami a `/subscription`-t és az `/account`-ot is kint tartja. Az **egyetlen valóban tenant-hatókörű írás** — a cég-alapértelmezés — `blockWritesWhenReadOnly`-t kap **inline**, pontosan úgy, ahogy a két hitelesített invite-write. Teszt rögzíti mindhárom ágat. (2) **A feed `userId`-re szűr, nem csak `companyId`-re** — egy szinttel szigorúbban, mint a repó többi routere: ugyanazon cég munkatársa sem láthatja. A `companyId` mellette marad védelmi rétegnek, de a `companyId IS NULL` sorok is beleférnek: a `NotificationEvent.companyId` szándékosan nullable (N1.1), így egy céghez nem köthető, de a felhasználónak címzett értesítés különben egy lyukba esne. |
| **Talált probléma** | ⚠️ **A registry-alapértelmezés hiányzott a kapuból.** A `gates.ts` N1.5 óta *dokumentálta*, hogy „az opt-in kategóriák alapból kikapcsoltak", de a kód sor nélküli esetben feltétel nélkül `ALLOWED`-ot adott. Ma elérhetetlen (egyetlen szállított típus sincs `digest`/`marketing` kategóriában), de az N1.9 első digest-típusa mindenkinek kiment volna, aki soha nem lett megkérdezve — `marketing` esetén ez nem preferencia-hiba, hanem **hozzájárulási hiba**. Azért itt javítva, mert N1.7 az a mérföldkő, ami ezt az alapértelmezést *publikálja* a preferencia-API-n: egy képernyő, ami „Termékhírek: ki"-t mutat, miközben a kapu küld, rosszabb mindkét viselkedésnél külön-külön. Új suppression-ok: `not_opted_in` (megkülönböztetve a „kikapcsolta"-tól — `marketing` esetén ez maga a hozzájárulási nyom). Két regressziós teszt rögzíti. ⚠️ **Mobil layout-hiba a harang paneljében**, 375px-en méréssel megfogva: a trigger-hez `right-0`-val kötött 320px-es panel bal széle **−186px**-nél volt, és mivel a body nem görget vízszintesen, az a rész elérhetetlen. Telefonon viewport-hoz kötött (`fixed`), `sm`-től felfelé továbbra is a triggerhez. Mindkét töréspont méréssel ellenőrizve. |

### N1.7.1 — Remediációs mérföldkő  🟢 *az architektúra-review Critical/High találatai*

| | |
|---|---|
| **Miért** | Az N1.1–N1.7 lezárása után külső szemmel végzett adverzális architektúra-review (6 független reviewer + skeptikus refutációs kör) 52 találatot vetett fel, ebből 47 túlélte a refutációt. A Critical/High halmaz javítása **nem halasztható az N1.8 mögé**: az N1.8 pénzügyi kommunikáció, és a hibák többsége pont a kézbesítési út megfigyelhetőségét és megbízhatóságát érintette. |
| **Tartalom** | **C1** `providerMessageId` írása (az `EmailService` immár `EmailSendResult`-ot ad vissza) · **C2** valódi queue→worker→email→DLQ integrációs tesztek · **H1** érzékeny context-kulcsok (`resetLink`/`verifyLink`/`inviteLink`) redakciója a kézbesítés lezárultakor · **H2** suppression-kivétel a security-levelekre `complained` esetén + operátori feloldó végpont · **H3** atomi fan-out (claim + delivery sorok egy tranzakcióban) · **H4** Resend idempotency key · **H5** DLQ-fogyasztó és terminális `failed` állapot + delivery-szintű sweep. |
| **Kész, ha** | Minden Critical/High találat javítva, mindegyikre regressziós teszt; a 387 eredeti teszt zöld marad; a queue→worker vakfolt lefedve. |
| **Állapot (2026-08-01)** | **KÉSZ.** **407/407 teszt zöld** (28 fájl) — 20 új: `notificationPipeline.test.ts` 2 (valódi workerekkel), `notificationRecovery.test.ts` 18. Mindkét build, lint tiszta, `dist` elleni mount-ellenőrzés 6/6. Egyetlen meglévő teszt módosult (`resendWebhook.test.ts`): az a H2 által hibásnak ítélt viselkedést rögzítette, a szándéka (globális suppression, mandatory nem kerüli meg) egy nem-exempt típussal megmaradt. |
| **A legfontosabb tanulság** | ⚠️ **Az N1.6 kézbesítés-követés soha nem működött.** A `providerMessageId` oszlopot **egyetlen production kódút sem írta** — csak olvasta (`deliveryEvents.ts`). Minden Resend-esemény párosítatlan maradt, a `delivered`/`bounced`/`complained` állapotok elérhetetlenek voltak, és a `NotificationDelivery` örökre `sent`-ben ragadt. A 387 teszt azért nem vette észre, mert a `resendWebhook.test.ts` **kézzel seedelte** az oszlopot a fixture-be — így a webhook-oldal zölden tesztelt egy funkciót, aminek a write-oldala nem létezett. **Szabály ebből:** egy oszlopot, amit a rendszer join-kulcsként használ, tilos tesztfixture-ből beírni; a tesztnek a valódi író utat kell futtatnia. |
...
*Aktuális állapot (2026-08-01): **N1.1 – N1.7 kész**. A soron következő
mérföldkő az N1.8 (Stripe billing-értesítések); N1.9 – N1.11 terv szinten
rögzítve, implementáció nem kezdődött el.*
