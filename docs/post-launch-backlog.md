...
*Anna 2026-08-05-én az N1–N2-t **elfogadott korlátként** vette tudomásul; egyik
sem blokkolja a Slice 4 mergét. Itt vannak rögzítve, hogy ne felejtődjenek el.*

⚠️ **Az N4–N6 a Slice 5-tel érkezett, és még NINCS róluk döntés.** Itt vannak
felsorolva, hogy a Slice 5 reviewja ne szóbeli emlékezetre támaszkodjon; ha
bármelyik elfogadhatatlan, az a Slice 5 újranyitása, nem külön tétel.*
...
| N4 | **A Slice 5 checkout-kapuja kézbesítési sorrendtől függ.** ⚠️ *Döntésre vár.* A `payment_method.attached` a Checkout során is tüzel, tehát minden előfizetés-indítás kivált egyet, és gátlás nélkül az ügyfél EGY regisztrációra KÉT levelet kapna (`billing.subscription_created` + `billing.payment_method_updated`) — ez a K1 hibaosztály másik eseményen. A kapu azt kérdezi a Company sortól, hogy van-e MÁR élő előfizetés, de ezt a két mezőt (`stripeSubscriptionId`, `subscriptionStatus`) a `checkout.session.completed` kezelője írja. A Stripe a kártyát a session lezárása ELŐTT csatolja, tehát a szokásos esetben a mezők még üresek és a regisztráció helyesen néma; a sorrend viszont **nincs garantálva**, és fordított sorrendnél a kapu átengedi. Rendes lezárása állapotot igényelne, amit a terv §5 kizár („séma-változás nem várható"). Követhető: minden új előfizetőnél meg kell jelennie a „first card at checkout" warn-sornak — lásd [notification-rollout.md](notification-rollout.md) Slice 5 (3). | N1.8 Slice 5 |
| N5 | **A `billing.payment_method_updated` kikapcsolható, pedig biztonsági jellegű üzenet is.** ⚠️ *Döntésre vár.* A jóváhagyott §4 tábla `billing_receipts` kategóriába és `mandatory: false`-ra teszi — nyugtaként kezeli, és a Slice 5 ezt követi. „Új fizetési mód került a fiókodhoz" viszont az a levél, amiből egy ügyfél megtudja, hogy valaki hozzáfért a számlázásához, és aki a `billing_receipts`-et kikapcsolta, ezt nem kapja meg. A kompenzáció a szövegben van (a levél kifejezetten megkérdezi, hogy az ügyfél végezte-e a módosítást), a kategória azonban nem ezt a kockázatot mérlegeli. Az újranyitása kategória-döntés (`billing` + `mandatory: true`), nem kód-javítás. | N1.8 Slice 5 |
| N6 | **Csak kártya.** ⚠️ *Döntésre vár.* A `billing.payment_method_updated` kizárólag `type === "card"` fizetési módra megy ki: a jóváhagyott adatszerződés `{brand, last4}`, és a szöveg kártyát nevez meg. SEPA-mandátum, Link vagy PayPal csatolása **némán** (naplózva) elmarad. EUR/HUF piacon a SEPA reális, tehát ha a termék valaha engedi, ez a szelet nem fedi le. Külön típus vagy általánosított szöveg kell hozzá, nem itt. | N1.8 Slice 5 |
